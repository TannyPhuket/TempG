...
export async function generateHistory() {
  ...
  const updates = {};
  const startDay = 14; // 14 วันย้อนหลัง
  const interval = 5*60*1000; // ทุก 5 นาที
  const now = new Date();
  for(let d=startDay; d>=0; d--){
    const dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()-d, 8, 0, 0); // 8 โมง
    const dayEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate()-d, 17, 0, 0); // 17 โมง
    ...
    while(time <= dayEnd.getTime()){
      ...
      if(overTempCount<3 && Math.random()<0.1){ temp = parseFloat((8 + Math.random()*2).toFixed(1)); overTempCount++; }
